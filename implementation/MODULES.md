...
## Impl-3 — Admission Controller + Request Queue + KV Accounting

- `config.py` — *(변경)* `AdmissionConfig` dataclass 신설. admission 관련 placeholder (N_sat · ctx tier 경계 · deadband width · idle θ · KV capacity · queue capacity) 를 한 곳에 모음. *Impl-10-pre-2 — k_total dial step/max 영역 폐기 (sequence-parallel PIM 위 channel knob 무의미).*
...
- ~~`k_total.py`~~ — *Impl-10-pre-2 폐기*. Sequence-parallel PIM 위 임의 시점 한 mb 가 모든 채널 점유 (k 영원 k_aggregate) 의 자연 정합 — channel allocation freedom 의 *t_proj fit* 제약 의미 0.
- `admission.py` — `Admission.layer1` 이 RequestQueue 에서 KV capacity 허용 한도까지 req 를 모아 → balance (inter-AB · intra-A · pim-slack) → mfu_floor clamp → `MicroBatchSpec` 산출. ARCH §6.4 admission 표 정합.
...
- `micro_batch.py` — *(변경)* admission 의 결정 정보를 dispatch 까지 운반하는 필드 신설: `kv_rows_total` (decode reqs 의 kv_length 합), `current_layer_index` (forward pass 의 현재 layer). Impl-4 의 carry-over O4.1 (dispatcher PIM 의 placeholder default args) 가 해소됩니다. *Impl-10-pre-2 — k_total field 폐기 (sequence-parallel PIM 위 channel knob 제거).*
- `admission.py` — *(변경)* `MicroBatchSpec` 에 `kv_rows_total` 필드 추가. `layer1` 산출 시 decode_reqs 의 kv_length 를 합산하여 spec 에 담아 줍니다.
- `dispatcher.py` — *(의미 변경)* `micro_batches: dict[int, MicroBatch]` 멤버 + `register`/`unregister` API 신설. `_op_time` 의 PIM branch 가 이제 placeholder 가 아닌 *실제 mb.kv_rows_total* 로 op_time 을 산출합니다 (O4.1 해소). 미등록 mb 의 PIM dispatch 는 raise. *Impl-10-pre-2 — k_channels 매개변수 폐기 (PIMExecutor.k_aggregate property 위 hw config 위 derived).*
- `main_loop.py` — *(의미 변경)* `ADMISSION_TICK` body 가 `MicroBatchSpec` → `MicroBatch` 변환 (Q1 — 변환 시점) 후 `dispatcher.register(mb)` 호출 → `window.admit(mb_id)` → `dispatcher.tick()` 의 순서로 진행합니다. spec 의 결정 정보 (kv_rows_total) 이 dispatcher 까지 유실 없이 흐릅니다.
